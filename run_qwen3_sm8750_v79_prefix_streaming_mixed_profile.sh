...
# One-click SM8750/V79 QNN profiling for the prefix-streaming mixed policy.
#
...
SCHEME="prefix_streaming_mixed"
SCALE_DIR="${STREAMING_SCALE_DIR:-${REPO_ROOT}/artifacts/p1/streaming-full-mapzp}"
STREAMING_MANIFEST="${STREAMING_MANIFEST:-${SCALE_DIR}/streaming-train.json}"
PRECISION_MAP="${PRECISION_MAP:-${REPO_ROOT}/artifacts/p0/static_a8/mixed-precision-map.json}"
...
LOCAL_MODEL="${LOCAL_MODEL:-/tmp/qwen3-1.7B-prefix-streaming-mixed-sm8750-v79.bin}"
REMOTE_MODEL="${REMOTE_MODEL:-qwen3-1.7B-prefix-streaming-mixed-sm8750-v79.bin}"
...
[[ -f "${LOCAL_MODEL}" ]] || die "native streaming mixed context missing: ${LOCAL_MODEL}; compile the G32 V79 context first"
...
export NATIVE_CONTEXT_PROVENANCE="prefix-streaming mixed native G32 V79 context; SHA verified"
export OFFLINE_LOGITS_COSINE="0.895313"
export OFFLINE_TOP1_AGREEMENT="0.833333"
export OFFLINE_LOGITS_NMSE="0.207467"
