...
# Canonical one-command Qwen3-1.7B profiling solution for SM8750/V79.
#
...
#
# Every invocation creates a new timestamped directory under ~/llm_exp/results.
...
[[ "${LOCAL_CONTEXT_SHA}" == "${EXPECTED_CONTEXT_SHA}" ]] \
    || die "native V79 context SHA mismatch: expected ${EXPECTED_CONTEXT_SHA}, got ${LOCAL_CONTEXT_SHA}"
...

{
    ...
    echo "capture_context=SM8750 native V79"
...
if [[ "${PREPARE_DEVICE}" == "1" ]]; then
    ...
    if [[ "${REMOTE_CONTEXT_SHA}" != "${EXPECTED_CONTEXT_SHA}" ]]; then
        echo "Pushing 1.6 GiB native V79 context ..."
...
python3 "${REPO_ROOT}/scripts/qnn_accuracy_summary.py" \
    ...
    --output "${RESULT_ROOT}/qwen3-sm8750-v79-accuracy.json" \
    | tee "${RESULT_ROOT}/accuracy-summary.log"

echo "===== Fresh-process Optrace captures ====="
for graph in s32 s1; do
    host_prefix="${RESULT_ROOT}/qwen3-sm8750-v79-${graph}"
...
python3 "${REPO_ROOT}/scripts/qnn_profile_speed_summary.py" \
    "${benchmark_csv[@]}" \
    --s1-qhas "${RESULT_ROOT}/qwen3-sm8750-v79-s1-chrometrace_qnn_htp_analysis_summary.json" \
    --s32-qhas "${RESULT_ROOT}/qwen3-sm8750-v79-s32-chrometrace_qnn_htp_analysis_summary.json" \
    --output "${RESULT_ROOT}/qwen3-sm8750-v79-speed.json" \
    | tee "${RESULT_ROOT}/speed-summary.log"

FINAL_REPORT="${RESULT_ROOT}/qwen3-sm8750-v79-e2e-critical-path.html"
python3 "${REPO_ROOT}/scripts/qnn_e2e_critical_path_report.py" \
    --results-dir "${RESULT_ROOT}" \
    --speed-json "${RESULT_ROOT}/qwen3-sm8750-v79-speed.json" \
    --accuracy-json "${RESULT_ROOT}/qwen3-sm8750-v79-accuracy.json" \
    --output "${FINAL_REPORT}" \
