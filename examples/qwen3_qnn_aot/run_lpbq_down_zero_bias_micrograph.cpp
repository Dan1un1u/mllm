...
// Execute a cached layer-14 S=32 down-projection micrograph. In profiling-off
// mode the runner emits raw QnnGraph_execute timings; when the caller selects
// optrace through the environment it performs one capture without overriding
// the requested QNN profiling configuration.
...
MLLM_MAIN({
  ...
  auto& input_path = Argparse::add<std::string>("--input").help("S=32 native UInt8 input file.");
  ...
  for (int invocation = 0; invocation < total; ++invocation) {
    backend->graphExecute("model.0.s32", inputs, outputs);
  }
