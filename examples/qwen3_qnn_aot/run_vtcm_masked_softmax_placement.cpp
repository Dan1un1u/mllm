...
MLLM_MAIN({
  ...
  auto& mask_file = Argparse::add<std::string>("--mask_file").help("Raw U8 mask.");
  ...
  if (numerical_fixture.get()) {
    std::vector<mllm::Tensor> inputs{
        loadU8Tensor({1, 1, seq.get(), kContext}, scores_file.get()),
        loadU8Tensor({1, 1, seq.get(), kContext}, mask_file.get()),
    };
    std::vector<mllm::Tensor> outputs{
        mllm::Tensor::empty({1, 1, seq.get(), kContext}, mllm::kUInt8, mllm::kQNN).alloc(),
    };
  ...
  for (int32_t i = 0; i < kKvHeads; ++i) { inputs.push_back(makeZeroU8Tensor({1, 1, kHeadDim, kContext})); }
  for (int32_t i = 0; i < kKvHeads; ++i) { inputs.push_back(makeZeroU8Tensor({1, 1, kContext, kHeadDim})); }
