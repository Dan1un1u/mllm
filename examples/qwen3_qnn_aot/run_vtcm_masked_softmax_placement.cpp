...
MLLM_MAIN({
  ...
  for (int32_t head = 0; head < kHeads; ++head) {
    inputs.push_back(makeZeroU8Tensor({1, 1, seq.get(), kHeadDim}));
  }
  for (int32_t head = 0; head < kKvHeads; ++head) {
    inputs.push_back(makeZeroU8Tensor({1, 1, kHeadDim, kContext}));
  }
  for (int32_t head = 0; head < kKvHeads; ++head) {
    inputs.push_back(makeZeroU8Tensor({1, 1, kContext, kHeadDim}));
  }
