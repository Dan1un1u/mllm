 ...
 public:
  ...
  std::vector<Tensor> forward(const std::vector<Tensor>& inputs, const std::vector<AnyValue>& args) override {
    ...
    for (int h = 0; h < num_attention_heads_; ++h) {
      ...
      // The custom path is deliberately represented as a same-shape Add so
      // the QNN visitor can recognize a two-input fused masked Softmax while
      // preserving every unrelated Add. The op consumes causal_mask as a
      // predicate; it does not numerically add its tiny U8 sentinel.
