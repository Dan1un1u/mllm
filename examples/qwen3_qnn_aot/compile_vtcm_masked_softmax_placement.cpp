...
class VtcmMaskedSoftmaxPlacementGraph final : public mllm::nn::Module {
 public:
  explicit VtcmMaskedSoftmaxPlacementGraph(const std::string& name) : mllm::nn::Module(name) {}
...
MLLM_MAIN({
  ...
  const auto stem = "vtcm_masked_softmax_placement_s" + std::to_string(seq_arg.get());
  mllm::redirect(stem + ".mir", [&]() { mllm::print(ir); });
  qnn_aot_env.saveContext("context.0", output_context.get());
  qnn_aot_env.destroyContext("context.0");
  mllm::print("VTCM masked-softmax placement compilation completed: " + output_context.get());
  return 0;
});
