...
MLLM_MAIN({
  ...
  if (layer.get() != 5) {
    MLLM_ERROR_EXIT(mllm::ExitCode::kCoreError, "this prototype is contracted to Layer 5");
  }
  ...
  auto trace = [&](int seq_len) {
    auto inputs = qwen3_qnn_aot::block::makeTraceInputs(seq_len, kContextLength, cfg, params, layer.get());
