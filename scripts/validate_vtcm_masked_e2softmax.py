...
def integer_reference(scores: bytes, mask: bytes, seq: int, score_scale: float = SCORE_SCALE) -> bytearray:
    coefficient = max(1, min(255, int(score_scale * 1.4375 * 256.0 + 0.5)))
...
def float_reference(scores: bytes, mask: bytes, seq: int, score_scale: float = SCORE_SCALE) -> list[float]:
...
def generate(seq: int, output_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "e2_exponent_coefficient_q8": max(1, min(255, int(SCORE_SCALE * 1.4375 * 256.0 + 0.5))),
...
def analyze(seq: int, fixture_dir: Path, device_output_path: Path) -> dict[str, object]:
    ...
    exact = float_reference(scores, mask, seq)
