...
def fixture(seq: int, score_zero_point: int) -> tuple[bytearray, bytearray]:
...
def integer_reference(
    scores: bytes, mask: bytes, seq: int, score_scale: float = SCORE_SCALE, beta: float = 1.0
) -> bytearray:
...
def generate(seq: int, output_dir: Path, score_scale: float, score_zero_point: int, beta: float) -> dict[str, object]:
    output_dir.mkdir(parents=True, exist_ok=True)
    scores, mask = fixture(seq, score_zero_point)
    expected = integer_reference(scores, mask, seq, score_scale=score_scale, beta=beta)
    paths = {
...
def analyze(
    seq: int, fixture_dir: Path, device_output_path: Path, score_scale: float, beta: float
) -> dict[str, object]:
    ...
    exact = float_reference(scores, mask, seq, score_scale=score_scale, beta=beta)
    ...
    implementation_valid = mismatches == 0 and masked_nonzero == 0 and nonzero_rows == seq and top1_matches == seq
    return {
        "seq": seq,
        "score_scale": score_scale,
        "beta": beta,
...
def main() -> int:
    ...
    if args.mode == "generate":
        report = generate(
            args.seq, args.fixture_dir, args.score_scale, args.score_zero_point, args.beta
        )
    else:
        if args.device_output is None:
            parser.error("--device-output is required in analyze mode")
        report = analyze(args.seq, args.fixture_dir, args.device_output, args.score_scale, args.beta)
