...
tag="cache_only_w1024_s32_p19"
case_dir="${artifact_root}/contexts/${tag}"

case "${artifact_root}" in
  /mnt/d/llm_exp/models/qwen3_sm8750_v79/g32/cache_only_intermediate_qairt249/20260824_sha_w1024_p19) ;;
...
[[ -s "${control_root}/full_w1024_s32_p19.bin" && -s "${control_root}/manifest.json" ]] || {
  echo "accepted full-width control missing: ${control_root}" >&2
  exit 2
}
[[ ! -e "${case_dir}" ]] || { echo "refusing existing candidate: ${case_dir}" >&2; exit 2; }

mkdir -p "${repo_root}/tmp" "${artifact_root}/contexts"
stage="$(mktemp -d "${repo_root}/tmp/cache_only_intermediate_compile.XXXXXX")"
case "${stage}" in
  "${repo_root}"/tmp/cache_only_intermediate_compile.*) ;;
...
rm -rf "${stage}"
trap - ERR INT TERM
echo "Cache-only intermediate context complete: ${case_dir}"
