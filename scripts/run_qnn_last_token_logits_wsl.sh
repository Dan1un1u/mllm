...
results_root="${RESULTS_ROOT:-/mnt/d/llm_exp/results/qwen3_sm8750_v79_last_token_logits_sha_qairt249_p19_20260824}"
...
remote="${REMOTE_DIR:-/data/local/tmp/mllm_last_token_logits_qairt249}"
resume_device="${RESUME_DEVICE:-0}"

case "${remote}" in
  /data/local/tmp/mllm_last_token_logits_qairt249) ;;
  *) echo "refusing unexpected REMOTE_DIR: ${remote}" >&2; exit 2 ;;
esac
case "${results_root}" in
  /mnt/d/llm_exp/results/qwen3_sm8750_v79_last_token_logits_sha_qairt249_p19_20260824) ;;
...
candidate_root="${artifact_root}/contexts/last_logits_w32_s32_p19"
for required in \
  "${control_root}/compact_w32_s32_p19.bin" \
  "${control_root}/manifest.json" \
  "${candidate_root}/last_logits_w32_s32_p19.bin" \
...
case "${staging}" in
  /mnt/d/llm_exp/results/qwen3_sm8750_v79_last_token_logits_sha_qairt249_p19_20260824.tmp.*) ;;
...
context_for() {
  case "$1" in
    control) printf '%s/compact_w32_s32_p19.bin\n' "${control_root}" ;;
    candidate) printf '%s/last_logits_w32_s32_p19.bin\n' "${candidate_root}" ;;
    *) return 2 ;;
  esac
}
...
execute_case_once() {
  ...
  adb_cmd shell "mkdir -p '${remote_dir}' && cd '${remote}' && env \
    ...
    ./initial-prefill-runner \
      ...
      --width 32 --logits '${logits}' --warmup '${warmup}' --iterations '${iterations}' \
...
if [[ "${resume_device}" == 1 ]]; then
  adb_cmd get-state >/dev/null
  adb_cmd shell "test -x '${remote}/initial-prefill-runner' && test -s '${remote}/control.bin' && test -s '${remote}/candidate.bin'"
else
  prepare_device
fi
record_provenance

python3 "${repo_root}/scripts/qnn_last_token_logits_audit.py" \
...
python3 "${repo_root}/scripts/qnn_last_token_logits_result.py" \
  --result-root "${staging}" --report "${staging}/summary.json" \
  --markdown "${staging}/SUMMARY.md"

mv "${staging}" "${results_root}"
trap - ERR INT TERM
echo "Last-token logits run complete: ${results_root}"
