...
remote="${REMOTE_DIR:-/data/local/tmp/mllm_lpbq_p_point_focus}"
points=(default 2 15 19)
projections=(gate_proj up_proj)

case "${remote}" in
  /data/local/tmp/mllm_lpbq_p_point_focus) ;;
  *) echo "refusing unexpected REMOTE_DIR: ${remote}" >&2; exit 2 ;;
esac
case "${results_root}" in
  /mnt/d/llm_exp/results/qwen3_sm8750_v79_lpbq_p_point_focus_20260821) ;;
...
staging="${repo_root}/build-lpbq-p-point-focus-staging"
mkdir -p "${staging}"
...
prepare_device() {
  ...
  for projection in "${projections[@]}"; do
    push_file "${source_root}/input_${projection}_s1_a8.raw" "${remote}/input_${projection}.raw"
...
execute_case() {
  local point="$1" projection="$2" run_tag="$3" warmup="$4" iterations="$5" profile="$6"
  local host_dir host_tmp remote_dir
  ...
  if [[ "${profile}" == optrace ]]; then
    adb_cmd pull "${remote_dir}/." "${host_tmp}/" >/dev/null
  fi
  [[ ! -e "${host_dir}" ]] || exit 2
...
for round in $(seq 1 10); do
  if (( round % 2 )); then order=("${points[@]}"); else order=(19 15 2 default); fi
  for projection in "${projections[@]}"; do
    for point in "${order[@]}"; do
      execute_case "${point}" "${projection}" "speed/round${round}" 50 1000 off
    done
  done
done
for projection in "${projections[@]}"; do
  for point in "${points[@]}"; do
    execute_case "${point}" "${projection}" optrace 0 1 optrace
  done
done
...
case "${publish}" in
  /mnt/d/llm_exp/results/qwen3_sm8750_v79_lpbq_p_point_focus_20260821.tmp.*) ;;
  *) exit 2 ;;
esac
mkdir -p "${publish}"
...
rm -rf "${staging}"
trap - ERR INT TERM
echo "LPBQ P-point focused run complete: ${results_root}"
