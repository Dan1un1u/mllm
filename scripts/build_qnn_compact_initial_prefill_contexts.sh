...
case "${artifact_root}" in
  /mnt/d/llm_exp/models/qwen3_sm8750_v79/g32/compact_initial_prefill_qairt249/20260823_sha_boolmask) ;;
...
[[ -s "${model_config}" && -s "${aot_config}" ]] || { echo "config missing" >&2; exit 2; }
...
for variant in full compact; do
  ...
  stage="$(mktemp -d "${repo_root}/tmp/compact_initial_prefill_compile.XXXXXX")"
  case "${stage}" in
    "${repo_root}"/tmp/compact_initial_prefill_compile.*) ;;
    *) echo "unexpected staging path: ${stage}" >&2; exit 2 ;;
  esac
...
echo "Compact initial-prefill contexts complete: ${artifact_root}/contexts"
